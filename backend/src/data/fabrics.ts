...
/**
 * Validate if a color exists for a given material and fabric type
 */
export function isValidColor(material: string, fabricType: string, color: string): boolean {
    const colors = getFabricColors(material, fabricType);
    return colors.includes(color);
}

/**
 * Get all available materials with their fabric types (for dropdown generation)
 */
export function getMaterialsWithTypes(): { material: string; types: string[] }[] {
    return getMaterials().map(material => ({
        material,
        types: getFabricTypes(material),
    }));
}
