...
async function main() {
    ...
    for (const bar of bottomBars) {
        const item = await prisma.inventoryItem.create({
            data: {
                category: client_1.InventoryCategory.BOTTOM_BAR,
                itemName: 'Bottom Bar',
                colorVariant: bar.color,
                quantity: bar.quantity,
                unitType: client_1.UnitType.UNITS,
                minStockAlert: 10,
            },
        });
        await prisma.inventoryTransaction.create({
    ...
    for (const motor of motors) {
        const item = await prisma.inventoryItem.create({
            data: {
                category: client_1.InventoryCategory.MOTOR,
                itemName: motor.name,
                colorVariant: null,
    ...
    for (const chain of chains) {
        const item = await prisma.inventoryItem.create({
            data: {
                category: client_1.InventoryCategory.CHAIN,
                itemName: chain.name,
                colorVariant: null,
